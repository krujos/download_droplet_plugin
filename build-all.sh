...
if [[ "$1" = "release" ]] ; then
	...
	if [ $? -eq 0 ] ; then
		...
		`sed -i .bak -e "s/Major:.*/Major: $MAJOR,/" \
			...
			-e "s/Build:.*/Build: $BUILD,/" command/download_droplet_cmd.go`
	fi
fi

GOOS=linux GOARCH=amd64 go build
LINUX64_SHA1=`cat download_droplet_plugin | openssl sha1`
mkdir -p bin/linux64
mv download_droplet_plugin bin/linux64

GOOS=darwin GOARCH=amd64 go build
OSX_SHA1=`cat download_droplet_plugin | openssl sha1`
mkdir -p bin/osx
mv download_droplet_plugin bin/osx

GOOS=windows GOARCH=amd64 go build
WIN64_SHA1=`cat download_droplet_plugin.exe | openssl sha1`
mkdir -p bin/win64
mv download_droplet_plugin.exe bin/win64
